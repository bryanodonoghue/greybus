...
int gb_svc_timesync_authoritative(struct gb_svc *svc, u64 *frame_time, u8 count)
{
	struct gb_connection *connection = svc->connection;
...
int gb_svc_timesync_wd_pins_acquire(struct gb_svc *svc, u32 strobe_mask)
{
	struct gb_connection *connection = svc->connection;
	struct gb_svc_timesync_wd_pins_acquire_request request;

	request.strobe_mask = cpu_to_le32(strobe_mask);
	return gb_operation_sync(connection,
				 GB_SVC_TYPE_TIMESYNC_WD_PINS_ACQUIRE,
				 &request, sizeof(request),
				 NULL, 0);
}
EXPORT_SYMBOL_GPL(gb_svc_timesync_wd_pins_acquire);

int gb_svc_timesync_wd_pins_release(struct gb_svc *svc)
{
	struct gb_connection *connection = svc->connection;

	return gb_operation_sync(connection,
				 GB_SVC_TYPE_TIMESYNC_WD_PINS_RELEASE,
				 NULL, 0, NULL, 0);
}
EXPORT_SYMBOL_GPL(gb_svc_timesync_wd_pins_release);
