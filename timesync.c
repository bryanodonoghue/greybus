...
static void gb_timesync_teardown(void)
{
	...
	gb_svc_timesync_wd_pins_release(svc);
...
static void gb_timesync_enable(void)
{
	...
	/*
	 * Get access to the wake-detect pins in the AP and SVC
	...
	ret = gb_svc_timesync_wd_pins_acquire(svc,  gb_timesync_strobe_mask);
	if (ret) {
		dev_err(&svc->dev,
			"gb_svc_timesync_wd_pins_acquire %d\n", ret);
...
/*
 * Send an SVC initiated wake-detect 'ping' to each TimeSync participant.
 * Get the frame-time from each participant associated with the wake-detect
 * ping.
 */
...
int gb_timesync_interface_add(struct gb_interface *interface)
{
	...
	gb_timesync_strobe_mask |= 1 << interface->device_id;
