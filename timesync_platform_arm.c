...
{
	int (*platform_bus_lock)(enum arche_platform_state state);

	platform_bus_lock =
		(void *)kallsyms_lookup_name("arche_platform_change_state");
	if (platform_bus_lock)
		return platform_bus_lock(ARCHE_PLATFORM_STATE_TIME_SYNC);
	return 0;
}
...
void gb_timesync_platform_unlock_bus(void)
{
	...
	platform_bus_lock =
		(void *)kallsyms_lookup_name("arche_platform_change_state");
	if (platform_bus_lock)
		platform_bus_lock(ARCHE_PLATFORM_STATE_ACTIVE);
}
...
{
	gb_timesync_platform_clock_disable();
	if (gb_timesync_clock_regs)
